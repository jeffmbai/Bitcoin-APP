...
type ThisMonthScreenProps = ScreenWithTranslation;


function ThisMonthScreen({ i18n, navigation }: ThisMonthScreenProps) {
  ...
  useEffect(() => {
    ...
    let listener = navigation.addListener('didFocus', async () => {
      ...
      let qs = await firestore().collection("models").orderBy("votes", "desc").get()
...
export default withTranslation()(withNavigation(ThisMonthScreen))
